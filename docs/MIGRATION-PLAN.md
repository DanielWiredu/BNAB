...
- **Windows/IIS decision (confirmed):** app + worker run as Windows Services behind an IIS reverse proxy; Redis via **Memurai** (Redis-compatible Windows service) or WSL2/Docker. Runbook in `DEPLOYMENT.md`.
...
### 3.7 Reports & exports (ADR-007 — REVISED 2026-07-07)

**Decision (confirmed):** build our **own native reports**; do not depend on `rptlams.gdlcwave.com`. The earlier "keep deep-linking" stance is superseded.
- Report views (cost sheets, payroll, loan statements) are rebuilt as server-rendered print-CSS pages and/or PDFs (Playwright / `@react-pdf`) inside the app, replacing each legacy `.aspx` deep link as its owning module is migrated.
- `REPORT_APP_URL` remains only as a temporary fallback for reports not yet rebuilt, retired per-report.
...
**Answered (2026-07-07):** ClientPortal pages out of scope; 2FA/external logins parked; hosting = Windows/IIS (ADR-005 + `DEPLOYMENT.md`); production DB = **`bnabdb`**. Remaining open items #5–#7 below are non-blocking for Phase 0/1.

1. **ClientPortal pages** (`LabourAdvice`, `Daily/Weekly/MonthlySubmission`) are empty stubs — ✅ out of scope.
