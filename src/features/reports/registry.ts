...
const REPORTS: ReportDef[] = [
  ...
  // ── Worker List (vwWorkerList) ────────────────────────────────────────────
  {
    ...
    // Legacy WORKER LIST: grouped by trade group ("Group"), columns
    // WorkerID · Name · Phone · SSF No · NHIS · DOB · Trade type · Sex · flags ·
    // Reg Date · Ezwich (matches vwWorkerList order by TradegroupNAME, SName).
    group: { key: "TradegroupNAME", label: "Group" },
    columns: [
      { key: "WorkerID", label: "Worker ID", width: 12 },
      { key: "Name", label: "Name", width: 26, value: (r) => `${r["SName"] ?? ""} ${r["OName"] ?? ""}`.trim() },
      { key: "PhoneNo", label: "Phone No", width: 14 },
      { key: "SSFNo", label: "SSF No", width: 18 },
      { key: "NHIS", label: "NHIS", width: 14 },
      { key: "Date_Birth", label: "Date of Birth", format: "date", width: 14 },
      { key: "TradetypeNAME", label: "Trade Type", width: 20 },
      { key: "Sex", label: "Sex", align: "center", width: 6 },
      { key: "flags", label: "Flags", align: "center", width: 8 },
      { key: "RegDate", label: "Reg Date", format: "date", width: 14 },
      { key: "ezwichid", label: "Ezwich", width: 16 },
    ],
    query: workerList,
  },
