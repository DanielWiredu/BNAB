...
export interface WeeklyInitial {
  requestNo: string;
  companyId: number;
  workerId: string;
  workerName: string;
...
export function WeeklyEditor({
  ...
  const form = useForm<Values>({
    resolver: zodResolver(weeklyReqSchema) as unknown as Resolver<Values>,
    defaultValues: {
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    if (isEdit) {
      const res = await updateWeeklyReq(values);
      if (res.ok) {
    ...
    const res = await createWeeklyReq(values);
    if (res.ok) {
      toast.success("Requisition saved.");
      router.push(`/operations/weekly/${res.data.reqNo}`);
    } else toast.error(res.error);
  });

  async function onConfirm() {
    const res = await confirmWeeklyReq(initial.requestNo);
    if (res.ok) {
  ...
  async function onAllowDup() {
    const res = await allowDuplicateShift(initial.requestNo);
    if (res.ok) toast.success("Duplicate shift allowed.");
    else toast.error(res.error);
  }
    ...
    <div className="space-y-6">
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          {initial.confirmed && <span className="rounded-md bg-[var(--muted)] px-2 py-1 text-sm">Confirmed</span>}
          <Button type="button" variant="outline" onClick={() => router.push("/operations/weekly")}>
            Return
          </Button>
          {isEdit && canSave && (
            <>
              <Button type="button" variant="outline" onClick={onAllowDup} disabled={approved}>
                Allow Duplicate Shift
              </Button>
      ...
      {/* Work days are only available once the requisition exists (edit mode). */}
      <WorkDaysGrid
